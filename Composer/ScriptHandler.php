...
class ScriptHandler
{
    ...
    public static function install(CommandEvent $event)
    {
        ...
        if (!is_dir($appDir)) {
            printf(
                'The symfony-app-dir (%s) specified in composer.json was not found in %s, can not build fontawesome '.
                'file.%s',
                $appDir,
                getcwd(),
                PHP_EOL
        ...
        static::executeCommand($event, $appDir, 'ekyna:fontawesome:install', $options['process-timeout']);
    }
